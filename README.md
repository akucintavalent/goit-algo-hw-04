...
Скрипт рекурсивно обходить вихідну директорію, копіює всі знайдені файли у директорію призначення та розкладає їх по піддиректоріях відповідно до розширення файлу.

- Аргументи командного рядка: шлях до вихідної директорії (обов’язково) і шлях до директорії призначення (необов’язково).
- Якщо директорію призначення не вказано, використовується `dist` у поточній директорії запуску.
- Для файлів без розширення використовується піддиректорія `no_extension`.
- Під час обходу директорія призначення пропускається (щоб уникнути рекурсивного копіювання `dist` у `dist`, якщо вона розміщена всередині `src`).

Запуск:

```bash
python task1_recursive_copy.py <src>
python task1_recursive_copy.py <src> <dst>
```

## Завдання 2. Фрактал «сніжинка Коха»

Скрипт будує фрактал «сніжинка Коха» за допомогою рекурсивної функції, яка для кожного відрізка на рівні рекурсії замінює його на 4 відрізки зі згинами на кути 60° і -120°.

- Користувач задає рівень рекурсії (order) через введення з клавіатури.
- Для візуалізації використовується модуль `turtle`.
...
Timsort є ефективнішим, тому що поєднує переваги сортування вставками та сортування злиттям. Сортування вставками добре працює на малих або майже впорядкованих частинах масиву, а сортування злиттям забезпечує стабільну складність `O(n log n)` для великих наборів даних.

Timsort спочатку знаходить уже впорядковані фрагменти даних, які називаються `runs`. Якщо такі фрагменти невеликі, алгоритм може ефективно обробити їх за принципом сортування вставками. Після цього впорядковані частини об'єднуються подібно до merge sort.

Таким чином, Timsort зберігає масштабованість сортування злиттям, але додатково виграє на малих, уже відсортованих або частково відсортованих ділянках завдяки ідеї сортування вставками.

Емпіричні результати підтверджують це: на випадкових і зворотно відсортованих даних Timsort має високу кореляцію з `O(n log n)`, а на вже відсортованих даних працює ще швидше, оскільки використовує наявний порядок елементів. Саме тому вбудовані `sorted()` і `list.sort()` у Python зазвичай ефективніші за власні реалізації merge sort або insertion sort.
